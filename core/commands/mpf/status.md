...
2. Query `linear_api` for team Rihm to get current ticket states.
...
If `linear_api` tools are unavailable or the query fails, display: "Linear status unavailable. Run `mpf:sync-linear` for detailed tracking."
